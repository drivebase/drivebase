...
func (_q *RoleQuery) QueryWorkspace() *WorkspaceQuery {
...
func (_q *RoleQuery) QueryMembers() *WorkspaceMemberQuery {
...
func (_q *RoleQuery) WithWorkspace(opts ...func(*WorkspaceQuery)) *RoleQuery {
	query := (&WorkspaceClient{config: _q.config}).Query()
...
func (_q *RoleQuery) WithMembers(opts ...func(*WorkspaceMemberQuery)) *RoleQuery {
	query := (&WorkspaceMemberClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *RoleQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Role, error) {
	...
	if query := _q.withWorkspace; query != nil {
		if err := _q.loadWorkspace(ctx, query, nodes, nil,
			func(n *Role, e *Workspace) { n.Edges.Workspace = e }); err != nil {
			return nil, err
		}
	}
	...
	if query := _q.withMembers; query != nil {
		if err := _q.loadMembers(ctx, query, nodes,
			func(n *Role) { n.Edges.Members = []*WorkspaceMember{} },
			func(n *Role, e *WorkspaceMember) { n.Edges.Members = append(n.Edges.Members, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *RoleQuery) loadWorkspace(ctx context.Context, query *WorkspaceQuery, nodes []*Role, init func(*Role), assign func(*Role, *Workspace)) error {
...
func (_q *RoleQuery) loadMembers(ctx context.Context, query *WorkspaceMemberQuery, nodes []*Role, init func(*Role), assign func(*Role, *WorkspaceMember)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[uuid.UUID]*Role)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(workspacemember.FieldRoleID)
	}
	query.Where(predicate.WorkspaceMember(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(role.MembersColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
...
func (_q *RoleQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := _q.ctx.Fields; len(fields) > 0 {
		...
		if _q.withWorkspace != nil {
			_spec.Node.AddColumnOnce(role.FieldWorkspaceID)
		}
	}

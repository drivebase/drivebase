...
type ApiToken struct {
	...
	// DisplayToken holds the value of the "display_token" field.
	DisplayToken string `json:"display_token,omitempty"`
	// Scopes holds the value of the "scopes" field.
	Scopes []string `json:"scopes,omitempty"`
	// ProviderScopes holds the value of the "provider_scopes" field.
	ProviderScopes []apitoken.ProviderScope `json:"provider_scopes,omitempty"`
...
func (*ApiToken) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		switch columns[i] {
		case entapitoken.FieldScopes, entapitoken.FieldProviderScopes:
			values[i] = new([]byte)
		case entapitoken.FieldName, entapitoken.FieldTokenHash, entapitoken.FieldDisplayToken:
...
func (_m *ApiToken) assignValues(columns []string, values []any) error {
	...
	for i := range columns {
		...
		case entapitoken.FieldDisplayToken:
			if value, ok := values[i].(*sql.NullString); !ok {
				return fmt.Errorf("unexpected type %T for field display_token", values[i])
			} else if value.Valid {
				_m.DisplayToken = value.String
			}
		...
		case entapitoken.FieldProviderScopes:
			if value, ok := values[i].(*[]byte); !ok {
				return fmt.Errorf("unexpected type %T for field provider_scopes", values[i])
			} else if value != nil && len(*value) > 0 {
				if err := json.Unmarshal(*value, &_m.ProviderScopes); err != nil {
					return fmt.Errorf("unmarshal field provider_scopes: %w", err)
				}
			}
...
func (_m *ApiToken) String() string {
	...
	builder.WriteString(", ")
	builder.WriteString("display_token=")
	builder.WriteString(_m.DisplayToken)
	builder.WriteString(", ")
	builder.WriteString("scopes=")
	builder.WriteString(fmt.Sprintf("%v", _m.Scopes))
	builder.WriteString(", ")
	builder.WriteString("provider_scopes=")
	builder.WriteString(fmt.Sprintf("%v", _m.ProviderScopes))
	builder.WriteString(", ")

...
function toNodeReadable(value: unknown): Readable {
  ...
  if (isWebReadableStream(value)) {
    return Readable.fromWeb(value as unknown as import("node:stream/web").ReadableStream<any>);
  }
...
export class S3Provider implements IStorageProvider {
  ...
  async move(options: MoveOptions): Promise<void> {
    ...
    try {
      if (options.remoteId.endsWith("/")) {
        ...
        const sourceBase = sourcePrefix.slice(0, -1).split("/").filter(Boolean).pop() ?? "";
        const destinationPrefix = folderKey(options.newParentId, options.newName ?? sourceBase);
        await this.copyPrefix(sourcePrefix, destinationPrefix, client, bucket);
        await this.delete({ remoteId: sourcePrefix, isFolder: true });
        return;
      }

      const originalName = options.remoteId.split("/").filter(Boolean).pop() ?? options.remoteId;
      const destinationKey = joinKey(options.newParentId, options.newName ?? originalName);
  ...
  async copy(options: CopyOptions): Promise<string> {
    ...
    try {
      if (options.remoteId.endsWith("/")) {
        const sourcePrefix = normalizePrefix(options.remoteId);
        const sourceBase = sourcePrefix.slice(0, -1).split("/").filter(Boolean).pop() ?? "folder";
        const destinationPrefix = folderKey(options.targetParentId, options.newName ?? sourceBase);
        await this.copyPrefix(sourcePrefix, destinationPrefix, client, bucket);
        return destinationPrefix;
      }

      const originalName = options.remoteId.split("/").filter(Boolean).pop() ?? options.remoteId;
      const destinationKey = joinKey(options.targetParentId, options.newName ?? originalName);
      await this.copySingle(options.remoteId, destinationKey, client, bucket);
  ...
  async getFolderMetadata(remoteId: string): Promise<FolderMetadata> {
    ...
    try {
      ...
      return {
        ...
        name: normalizedFolderId.slice(0, -1).split("/").pop() ?? normalizedFolderId,
  ...
  private async copySingle(sourceKey: string, destinationKey: string, client: S3Client, bucket: string) {
    await client.send(
  ...
  private async copyPrefix(sourcePrefix: string, destinationPrefix: string, client: S3Client, bucket: string) {
    let continuationToken: string | undefined;

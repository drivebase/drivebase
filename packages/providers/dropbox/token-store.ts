...
export type PersistFn = (next: DropboxTokens) => Promise<void> | void;
...
export class TokenStore {
  ...
  async getAccessToken(): Promise<string> {
    ...
    if (!this.tokens.refreshToken) {
      throw new AuthError(
        "dropbox access token expired and no refresh_token available",
      );
    }
  ...
  async forceRefresh(): Promise<string> {
    if (!this.tokens.refreshToken) {
      throw new AuthError("dropbox 401 but no refresh_token available");
    }
  ...
  private async doRefresh(): Promise<string> {
    ...
    const next = await dropboxOAuth.refreshToken({

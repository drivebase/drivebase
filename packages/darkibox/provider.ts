...
export class DarkiboxProvider implements IStorageProvider {
	...
	private async apiGet<T>(
		...
		try {
			...
			throw new ProviderError("darkibox", "Darkibox API request failed", {
				endpoint,
	...
	async uploadFile(
		...
		try {
			...
			throw new ProviderError("darkibox", "Upload request failed", {
				op: "uploadFile",
				remoteId,
				error: toJsonSafeError(error),
			});
		...
		try {
			...
			throw new ProviderError("darkibox", "Failed to parse upload response", {
				op: "uploadFile",
				remoteId,
				error: toJsonSafeError(error),
			});
	...
	async downloadFile(remoteId: string): Promise<ReadableStream> {
		...
		if (dlResponse.useDirectDownload && dlResponse.downloadUrl) {
			...
			try {
				...
				throw new ProviderError("darkibox", "Download request failed", {
					op: "downloadFile",
					remoteId,
					error: toJsonSafeError(error),
				});

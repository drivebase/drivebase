...
export class GoogleDriveProvider implements IStorageProvider {
  ...
  async getAccountInfo(): Promise<{ email?: string; name?: string }> {
    ...
    try {
      ...
      throw new ProviderError("google_drive", "Failed to get account info", { error });
    }
  }
  // ... existing imports
  ...
  async uploadFile(remoteId: string, data: ReadableStream | Buffer): Promise<void> {
    ...
    try {
      ...
      const requestBody = Buffer.isBuffer(data)
        ? new Uint8Array(data)
        : data;
      ...
      if (error instanceof ProviderError) throw error;
      throw new ProviderError("google_drive", "Failed to upload file", { error });
    }
  }

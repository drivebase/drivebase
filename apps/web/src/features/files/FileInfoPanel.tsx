...
export function FileInfoPanel({ fileId }: FileInfoPanelProps) {
  ...
  if (error || !file) {
    return <div className="text-sm text-destructive">Failed to load file details.</div>;
  }
    ...
    <div className="space-y-5">
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-xs text-muted-foreground">{formatFileTypeLabel(file.mimeType)}</p>
        </div>
      </div>

      <div className="space-y-2 rounded-md border p-3">
        <div className="text-xs font-semibold text-muted-foreground uppercase tracking-wide">Storage Provider</div>
        <div className="flex items-center gap-2 text-sm">
          <ProviderIcon type={file.provider.type} className="h-4 w-4" />
          <span>{file.provider.name}</span>
          <span className="text-muted-foreground">({file.provider.type.replace("_", " ")})</span>
        </div>
      </div>

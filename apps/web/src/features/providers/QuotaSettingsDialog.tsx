...
interface QuotaSettingsDialogProps {
  ...
  isSaving?: boolean;
  onSave: (input: { id: string; quotaTotal: number; quotaUsed: number }) => Promise<void>;
}
...
export function QuotaSettingsDialog({
  ...
  const handleSave = async () => {
    ...
    setError(null);
    await onSave({ id: provider.id, quotaTotal: totalBytes, quotaUsed: usedBytes });
  };
  ...
  return (
    <Dialog open={isOpen} onOpenChange={(open) => !open && onClose()}>
      <DialogContent className="sm:max-w-md">
        ...
        <DialogFooter>
          <Button type="button" onClick={handleSave} disabled={!provider || isSaving}>

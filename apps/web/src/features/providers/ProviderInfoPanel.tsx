	...
	return (
		<div className="space-y-5">
			...
			{provider.accountEmail || provider.accountName ? (
				<div className="space-y-2 rounded-md border p-3">
					...
					{provider.accountName ? (
						<div className="text-sm">{provider.accountName}</div>
					) : null}
			...
			<div className="space-y-2">
				<div className="text-xs border-b pb-2 font-semibold text-muted-foreground uppercase tracking-wide">
					Provider Configuration
				</div>
				<div className="space-y-4">
					{provider.configPreview.map(
						(entry: { key: string; value: string; isSensitive: boolean }) => (
							<div key={entry.key} className="flex flex-col gap-2 text-sm">
								<h1 className="text-muted-foreground">{entry.key}</h1>
								<span className="break-all font-mono">
									{entry.value}
									{entry.isSensitive ? (
										<Shield className="inline h-3.5 w-3.5 ml-1 text-muted-foreground" />
									) : null}
								</span>
							</div>
						),
			...
			<div className="text-xs text-muted-foreground">
				Last synced:{" "}
				{provider.lastSyncAt
					? format(new Date(provider.lastSyncAt), "MMM dd, yyyy HH:mm")
					: "Never"}
			</div>
		</div>
	);

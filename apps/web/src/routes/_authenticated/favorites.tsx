...
import { useStarFolder, useStarredFolders, useUnstarFolder } from "@/hooks/useFolders";
...
function FavoritesPage() {
  ...
  const { data: starredFoldersData, fetching: foldersFetching } = useStarredFolders();
  ...
  useEffect(() => {
    setFolders((starredFoldersData?.starredFolders || []) as FolderItemFragment[]);
  }, [starredFoldersData?.starredFolders]);

  const handleToggleFileFavorite = async (file: FileItemFragment) => {
    const currentStarred = files.find((f) => f.id === file.id)?.starred ?? file.starred;
  ...
  if (!filesFetching && !foldersFetching && folders.length === 0 && files.length === 0) {
    return (

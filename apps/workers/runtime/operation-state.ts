...
export async function maybeFinalizeOperation(args: {
  ...
  // awaiting_conflict jobs are suspended pending user decision — treat as pending
  const pending =
    (counts.get("queued") ?? 0) +
  ...
  if (updated.length === 0) return; // another worker already finalized
  console.info("[worker operation finalize]", {
    operationId,
    terminal,
    counts: Object.fromEntries(counts),
  });
  await publishProgress(pub, {
    kind: "operation",
    operationId,
    status: terminal,
  });

  // After a move-transfer succeeds, delete source nodes server-side so the
  // delete happens even if the browser is closed before the client can react.
  if (terminal === "succeeded" && args.getQueue) {
    await maybeEnqueueSourceDelete(db, operationId, args.getQueue).catch((err) =>
      console.error("[worker source-delete]", { operationId, err }),
    );
  }
}

async function maybeEnqueueSourceDelete(
  db: Db,
  operationId: string,
  getQueue: (name: QueueName) => Promise<Queue>,
): Promise<void> {

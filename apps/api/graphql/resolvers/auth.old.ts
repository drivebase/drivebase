import type { MutationResolvers, QueryResolvers, AuthResponseResolvers } from "../generated/types";
...
export const authMutations: MutationResolvers = {
  register: async (_parent, args, context) => {
    ...
    try {
      ...
      const result = await authService.register(args.input.email, args.input.password, role, context.ip);
      console.log("[RESOLVER] Register result:", result ? { userId: result.user.id, hasToken: !!result.token } : "null");
      return result;
    } catch (error) {
  ...
  login: async (_parent, args, context) => {
    ...
    try {
      ...
      const result = await authService.login(args.input.email, args.input.password, context.ip);
      console.log("[RESOLVER] Login result:", result ? { userId: result.user.id, hasToken: !!result.token } : "null");
      return result;
    } catch (error) {
  ...
  changePassword: async (_parent, args, context) => {
    const user = requireAuth(context);
    const authService = new AuthService(context.db);
    await authService.changePassword(user.userId, args.input.currentPassword, args.input.newPassword);
    return true;
  },
  ...
  resetPassword: async (_parent, args, context) => {
    const authService = new AuthService(context.db);
    await authService.resetPassword(args.input.email, args.input.otp, args.input.newPassword);
    return true;
  },
};

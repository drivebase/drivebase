...
import { AuthenticationError, AuthorizationError, ValidationError } from "@drivebase/core";
...
export const resolvers = {
  Query: {
    ...
    users: async (_: unknown, args: { limit?: number; offset?: number }, context: GraphQLContext) => {
    ...
    storageProviders: async (_: unknown, __: unknown, context: GraphQLContext) => {
      requireAuth(context);
      // TODO: Implement provider service
      return [];
    },

    folders: async (_: unknown, args: { parentPath?: string }, context: GraphQLContext) => {
      requireAuth(context);
  ...
  Mutation: {
    ...
    changePassword: async (
      ...
      const user = requireAuth(context);
      const authService = new AuthService(context.db);
      await authService.changePassword(user.userId, args.currentPassword, args.newPassword);
    ...
    deleteUser: async (_: unknown, args: { id: string }, context: GraphQLContext) => {
      requireRole(context, ["admin", "owner"]);
      const userService = new UserService(context.db);
    ...
    connectStorage: async (
      _: unknown,
      args: { input: { name: string; type: string; config: Record<string, unknown> } },
      context: GraphQLContext
    ) => {
      requireAuth(context);
      // TODO: Implement provider service

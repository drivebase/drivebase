...
export class AuthService {
   ...
   */
  async register(email: string, password: string, role: string, ipAddress: string) {
    ...
    if (!validation.valid) {
      logger.warn({ msg: "Password validation failed", message: validation.message });
    ...
    logger.debug({ msg: "User created", user: user ? { id: user.id, email: user.email } : "NULL" });
  ...
  async login(email: string, password: string, ipAddress: string) {
    ...
    logger.debug({ msg: "User found", user: user ? { id: user.id, email: user.email, role: user.role } : "not found" });

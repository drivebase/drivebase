...
export class FileService {
  ...
  async requestUpload(
    ...
    try {
      ...
      const parentId = folder?.remoteId ?? providerRecord.rootFolderId ?? undefined;
      ...
      const uploadUrl = uploadResponse.useDirectUpload ? uploadResponse.uploadUrl : proxyUrl;
  ...
  async requestDownload(fileId: string, userId: string) {
    ...
    try {
      ...
      const canUseDirectDownload =
        downloadResponse.useDirectDownload && Boolean(downloadResponse.downloadUrl);

      logger.debug({
        msg: "Download requested",
        fileId,
        useDirectDownload: canUseDirectDownload,
        downloadUrl: canUseDirectDownload ? downloadResponse.downloadUrl : proxyUrl
      });
  ...
  async renameFile(fileId: string, userId: string, newName: string) {
    ...
    try {
      ...
      logger.debug({ msg: "File renamed", fileId, oldName: file.name, newName: sanitizedName });
  ...
  async moveFile(fileId: string, userId: string, newFolderId?: string) {
    ...
    try {
      ...
      logger.debug({ msg: "File moved", fileId, oldPath: file.virtualPath, newPath: newVirtualPath });

      return updated;
    } catch (error) {
      logger.error({ msg: "Move file failed", userId, fileId, newFolderId, error });
      throw error;
    }
  }
  ...
  async getContents(path: string) {
    ...
    const [folder] = await this.db
      .select()
      .from(folders)
      .where(and(eq(folders.virtualPath, normalizedPath), eq(folders.isDeleted, false)))
      .limit(1);

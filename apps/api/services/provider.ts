...
import { getProviderRegistration, getSensitiveFields } from "../config/providers";
...
export class ProviderService {
  ...
  getProviderConfigPreview(
    ...
    const decryptedConfig = decryptConfig(providerRecord.encryptedConfig, sensitiveFields);

    return Object.entries(decryptedConfig).map(([key, value]) => {
      const isSensitive = sensitiveFields.includes(key);
      return {
        key,
        value: isSensitive ? this.maskSensitiveValue(value) : String(value ?? ""),
        isSensitive,
      };
    });
  }
  ...
  async connectProvider(
    ...
    const schema = registration.configSchema as { safeParse: (v: unknown) => { success: boolean; error?: { errors: unknown[] } } };
  ...
  async handleOAuthCallback(code: string, state: string) {
    ...
    const callbackUrl = this.buildCallbackUrl();
    const updatedConfig = await registration.handleOAuthCallback(config, code, callbackUrl);
    ...
    const maybeAccountInfo = (provider as {

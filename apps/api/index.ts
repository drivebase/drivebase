 ...
 */
const typeDefs = loadSchemaSync(join(import.meta.dir, "./graphql/schema/**/*.graphql"), {
...
function isExpectedAppErrorFromGraphQLLog(payload: unknown): boolean {
  const record = typeof payload === "object" && payload !== null ? (payload as any) : null;
...
async function handleOAuthCallback(request: Request): Promise<Response> {
  ...
  try {
    ...
    const frontendUrl = env.CORS_ORIGIN;
    return Response.redirect(`${frontendUrl}/providers?error=oauth_failed`, 302);
  }
}
...
async function handleUploadProxy(request: Request): Promise<Response> {
  ...
  try {
    ...
    const providerService = new ProviderService(db);
    const providerRecord = await providerService.getProvider(file.providerId, userId);
    ...
    logger.debug({ msg: "Streaming upload to provider", fileId, providerId: file.providerId });
    ...
    try {
      ...
      logger.error({ msg: "Failed to cleanup after upload failure", error: cleanupError, fileId });
    }

    const errorMessage = error instanceof Error ? error.message : String(error);
    return new Response(`Upload failed: ${errorMessage}`, { status: 500, headers: corsHeaders });
  }
}
...
async function handleDownloadProxy(request: Request): Promise<Response> {
  ...
  try {
    ...
    logger.debug({ msg: "Streaming download from provider", fileId, providerId: file.providerId });
...
const server = Bun.serve({
  ...
  async fetch(request) {
    ...
    if (request.method === "OPTIONS" && url.pathname === "/api/download/proxy") {
      return new Response(null, {
        headers: {
          "Access-Control-Allow-Origin": env.CORS_ORIGIN,

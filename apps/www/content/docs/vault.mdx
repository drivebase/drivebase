---
title: Vault (E2EE)
description: End-to-End Encrypted Storage Through Drivebase
---
...
In the Vault, all files are encrypted **client-side** (on your device) *before* they are sent to the Drivebase API and proxied to your configured storage provider (Local, S3, etc.). The storage provider receives only ciphertext, while Drivebase handles encrypted payloads and encrypted key material only.
...
Drivebase employs a zero-knowledge architecture for the Vault. Vault file ciphertext is stored at your configured storage provider, while Drivebase stores encrypted key material and metadata needed to route access. Neither can decrypt vault content without the user's passphrase-derived keys held client-side.
...
### Storage Proxying
Once chunks are encrypted locally on your device, they are sent through the Drivebase API. Drivebase proxies encrypted payloads to the final destination (your configured storage provider) and does not decrypt them in transit.

---
title: Vault (E2EE)
description: End-to-End Encrypted Storage in Drivebase
---

The **Vault** is a private, per-user encrypted file storage area inside Drivebase. Because Drivebase is a self-hosted solution, it's important to understand how your data is protected from the server infrastructure itself.

In the Vault, all files are encrypted **client-side** (on your device) *before* they are uploaded to the Drivebase server. The server stores only ciphertext and encrypted key material — it **never** sees the plaintext file content or your raw private key. The server merely acts as a proxy, securely transferring the encrypted data to your configured storage provider (Local, S3, etc.).
...
Drivebase employs a zero-knowledge architecture for the Vault. The server stores only encrypted key material and ciphertext. It cannot decrypt any vault file.
...
4. The File Key encrypts the actual **File Content** (AES-GCM 256-bit) before it leaves your device.
...
### Storage Proxying
Once the chunks are encrypted locally on your device, they are sent to the Drivebase server. The server acts as a proxy, moving these encrypted chunks to the final destination (your configured Storage Provider) without ever possessing the decryption keys.

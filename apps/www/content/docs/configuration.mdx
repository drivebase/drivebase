...
- `APP_URL`: The full URL where the app is accessible (e.g., `http://localhost:3000`).
...
### Web Client
- `VITE_PUBLIC_BASE_URL`: The public API base URL used by the browser (e.g., `http://localhost:4000`). The app appends `/graphql` automatically for GraphQL.

### Telemetry
- `DRIVEBASE_TELEMETRY`: Set to `false` to disable anonymous usage telemetry (default: `true`). See [Telemetry](/docs/telemetry) for details on what is collected.

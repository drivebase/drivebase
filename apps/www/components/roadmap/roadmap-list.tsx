...
export function RoadmapList({ items }: { items: RoadmapListItem[] }) {
  ...
  return (
    <div
      ref={containerRef}
      className="relative mx-auto max-w-4xl pb-16 sm:pb-24 lg:pb-32"
    >
      {/* Vertical line */}
      ...
      <div className="space-y-12">
        {items.map((item, index) => {
          const versionLabel =
            item.data.version?.trim() || item.data.title?.trim() || "Roadmap";
            ...
            >
              ...
              <div className="space-y-4">
                {item.data.features.map((feature, featureIndex) => {
                  const featureTitle = feature.title?.trim();
                  const featureDescription = feature.description?.trim();

                  if (!featureTitle && !featureDescription) return null;

                  return (
                    <div
                      key={`${item.url}-${featureTitle ?? featureDescription ?? featureIndex}`}
